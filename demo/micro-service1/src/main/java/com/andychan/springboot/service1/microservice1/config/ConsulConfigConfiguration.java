package com.andychan.springboot.service1.microservice1.config;

import com.andychan.springboot.service1.microservice1.service.ConfigDataService;
...
@Component
public class ConsulConfigConfiguration {


    private static final Logger log = LoggerFactory.getLogger(ConsulConfigConfiguration.class);

    @Autowired
    private ConfigDataService configDataServiceImpl;
    /**
    ...
    private void visitProps(String key, Object value, List<String> keyList) {
        if (value.getClass() == String.class || value.getClass() == JSONArray.class) {
            // 覆盖已有配置
            if (cover) {
                this.configDataServiceImpl.setKVValue(key, value.toString());
            } else {
    ...
    private void init() {
        ...
        List<String> keyList = this.configDataServiceImpl.getKVKeysOnly();
    ...
    private Map<String, Object> getProperties(String fixed) {
        ...
        if (fixed != null && !fixed.isEmpty()) {
            resource = new ClassPathResource("/config/application-" + fixed + ".properties");
        } else {
            resource = new ClassPathResource("/config/system-config.properties");
        }
